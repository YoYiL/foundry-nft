// SPDX-License-Identifier: MIT
pragma solidity ^0.8.18;
import {Test} from "forge-std/Test.sol";
import {MoodNft} from "src/MoodNft.sol";
import {console} from "forge-std/Test.sol";
import {DeployMoodNft} from "script/DeployMoodNft.s.sol";
import {Base64} from "@openzeppelin/contracts/utils/Base64.sol";

contract MoodNftIntegrationTest is Test {
    MoodNft moodNft;
    address public USER = makeAddr("user");
    string public constant BASE_URL = "data:application/json;base64,";
    string public constant HAPPY_SVG_IMAGE_URI =
        "data:image/svg+xml;base64,";
    string public constant SAD_SVG_IMAGE_URI =
        "data:image/svg+xml;base64,";
    bytes public SAD_SVG_URI;
    bytes public HAPPY_SVG_URI;
    DeployMoodNft public deployer;

    function setUp() public {
        deployer = new DeployMoodNft();
        moodNft = deployer.run();
        HAPPY_SVG_URI = abi.encodePacked(
            BASE_URL,
            Base64.encode(
                bytes(
                    abi.encodePacked(
                        '{"name: "',
                        moodNft.name(),
                        '", description: "An NFT that reflects your mood!", "attributes": [{"trait_type": "Mood", "value": 100}], "image": ',
                        HAPPY_SVG_IMAGE_URI,
                        '"}'
                    )
                )
            )
        );
        SAD_SVG_URI = abi.encodePacked(
            BASE_URL,
            Base64.encode(
                bytes(
                    abi.encodePacked(
                        '{"name: "',
                        moodNft.name(),
                        '", description: "An NFT that reflects your mood!", "attributes": [{"trait_type": "Mood", "value": 100}], "image": ',
                        SAD_SVG_IMAGE_URI,
                        '"}'
                    )
                )
            )
        );
    }

    function testViewTokenURIIntegration() public {
        vm.prank(USER);
        moodNft.mintNft();
        console.log(moodNft.tokenURI(0));
    }

    function testFlipMoodIntegration() public {
        vm.prank(USER);
        moodNft.mintNft();
        console.log("MoodNFT tokenURI(0) before flip: ", moodNft.tokenURI(0));
        vm.prank(USER);
        moodNft.flipMood(0);
        console.log("MoodNFT tokenURI(0) after flip: ", moodNft.tokenURI(0));
        console.log("Expected SAD_SVG_URI: ", string(SAD_SVG_URI));
        assert(
            keccak256(abi.encodePacked(moodNft.tokenURI(0))) ==
                keccak256(abi.encodePacked(SAD_SVG_URI))
        );
    }
}
